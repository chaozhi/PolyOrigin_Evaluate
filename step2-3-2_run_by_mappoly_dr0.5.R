...
telap=c("nfilt","tread","tcount","tpair","thmm","thmmerr","tprob","tsave")
...
for (dr in c(dr)){
  ...
  for (i in 1:7){
    ...
    for (rep in 1:nrep){
      ...
      thmm=system.time(
        map <- tryCatch({
          est_rf_hmm_sequential(input.seq = dat.seq,
                                start.set = 4,
                                thres.twopt = 5,
                                thres.hmm = 5,
      ...
      telap <- as.numeric(c(nfilt,tread[3],tcount[3],tpair[3],thmm[3],thmmerr[3],tprob[3],tsave[3]))
      res<-c(c(popsize,dr,rep),telap)
      print(res)
      write(res, file = sumfile,ncolumns = length(res),append = TRUE, sep = ",")
    }
